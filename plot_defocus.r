...
## Using ggplot2
min_of_data = round(log10(min(data1,na.rm=TRUE))) * (10^floor(log10(min(data1,na.rm=TRUE))))
max_of_data = round(log10(max(data1,na.rm=TRUE))) * (10^floor(log10(max(data1,na.rm=TRUE))))
step_of_data=10000
...
g1 = ggplot(data=data1,aes(x=data1$V1))+ geom_histogram(binwidth=1000,fill=barfill,colour=barlines)+
  scale_x_continuous(name="Defocus (A)",breaks=seq(min_of_data,max_of_data,step_of_data),limits=c(0,max_of_data))+
